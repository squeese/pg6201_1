...
export const Provider = ({ children }) => {
  ...
  const proxy = useRef(changeProxy({}));
  useEffect(() => {
    setReady(true);
...
export const Event = ({ onReady = noop, onChange = noop }) => {
  ...
  useEffect(() => {
    ...
    if (ready) {
      ...
      else if (prev.current !== state) onChange({ update, state });
      prev.current = state;
    }
  }, [state]);
  return null;
...
export const Float = ({ name, value, min = MIN, max = MAX, step = 0.1, children, ...props }) => {
  ...
  useEffect(() => {
    update(proxy => proxy[name].set(value));
...
export const Dropdown = ({ name, value, children }) => {
  const { state, update } = useContext(Context);
  useEffect(() => {
    update(proxy => proxy[name].set(value[0]));
    return () => update(proxy => proxy[name].delete());
  }, []);
...
export const Dictionary = ({ name, children }) => {
  ...
  useEffect(() => {
    if (!loaded.current)
      context.update(proxy => proxy[name].set({}));
    return () => {
  ...
  return (
    <Context.Provider
      value={{ ready, update, state: state[name] || {}}}
      children={children}
    />
  );
};
...
export const Vector = ({ name, value = [0, 0, 0], children, ...props }) => {
  const { ready, state, ...context } = useContext(Context);
  const active = useRef(true);
  const loaded = useRef(false);
  useEffect(() => {
    if (!loaded.current)
      context.update(proxy => proxy[name].set(value));
    return () => {
      active.current = false;
      context.update(proxy => proxy[name].delete());
    };
  ...
  return (
    <Context.Provider value={{ ready, update, state: state[name] || [] }}>
      {value.map(children)}
    </Context.Provider>
  );
};
